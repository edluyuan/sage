...
def pipeline(args):
    ...
    if args.mode == "inference":
        ...
        # ===== Instantiate unified JEPA gate (like MuJoCo) =====
        use_jepa_gate = bool(getattr(args, 'enable_jepa_gate', True))
        if use_jepa_gate:
            gate = SJEPAGate(
                env_id=args.task.env_name,
                encoder_ckpt=getattr(args, 'sj_encoder_ckpt', 'jepa/results/d4rl/maze2d-medium-v1/seed42/encoder_ema.pt'),
                ac_ckpt=getattr(args, 'sj_ac_ckpt', 'jepa/results-ac/d4rl/maze2d-medium-v1/seed42/ac_predictor_final.pt'),
            ...
            gate_cfg = GatingConfig(
                ...
                top_p=(getattr(args, 'gate_top_p', 0.8)),
                tau=(getattr(args, 'gate_tau', None)),
                lambda_pen=float(getattr(args, 'gate_lambda_pen', 0.01)),
        ...
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < args.task.max_path_length + 1:
                ...
                if args.guidance_type == "MCSS":
                    ...
                    obs_repeat = obs_norm_t.unsqueeze(1).repeat(1, args.planner_num_candidates, 1).view(-1, obs_dim)
                    ...
                    traj_bchd = traj.view(args.num_envs, args.planner_num_candidates, args.task.planner_horizon, planner_dim)
                    
                    # resample
                    with torch.no_grad():
                        v_td = critic(traj)
                        v_sum = v_td.view(args.num_envs, args.planner_num_candidates)
                    ...
                    if use_jepa_gate:
                        ...
                        idx = torch.argmax(v_masked, dim=-1)
                            #traj = traj_bchd[torch.arange(args.num_envs, device=args.device), best_idx]
                    else:
                        idx = torch.argmax(v_sum, -1)
                    traj = traj.reshape(args.num_envs, args.planner_num_candidates, args.task.planner_horizon, planner_dim)
                    traj = traj_bchd[torch.arange(args.num_envs), idx]
                ...
                if args.pipeline_type == "separate":
                    if args.use_diffusion_invdyn:
                        ...
                        # inverse dynamic
                        with torch.no_grad():
                            act = invdyn.predict(obs_norm_t, traj[:, 1, :]).cpu().numpy()
                else:
                    act = traj[:, 0, obs_dim:]
                    act = act.cpu().numpy()
